...
function AllResults({ setIsLoading, resultsData, setResultsData, setStep }) {
  const { authData } = useSelector(auth);
  const [results, setResults] = useState([]);

  const { term, grade, session } = resultsData;
    ...
    (async () => {
      try {
        ...
        const { data } = await axios.get(
          `${BASE_URL}/api/get-all-results/${grade}/${session}/${term}`,
          config
        );
